...
const MAX_ENEMY_LASER_SPEED = 20;
...
function createBoss($container) {
    ...
    $boss.src = "https://media.giphy.com/media/plNc70k90MpVlbNRfZ/giphy.gif";
    $boss.className = "boss";
...
function updateBossLaser(deltaTime, $container) {
    ...
    for (let i = 0; i < lasers.length; i++) {
        ...
        if (collision(rec1, rec2)) {
            GAME_STATE.playerLife = GAME_STATE.playerLife - 3;
...
let stop_shoot = false;
function updateBoss(deltaTime, $container) {
    const $boss = document.querySelector(".boss");
    if (!GAME_STATE.pauseOn && !GAME_STATE.gameWin && !GAME_STATE.gameOver) {
        if (!GOD_MOD && !stop_shoot) {
            stop_shoot = true;
            createBossLaser($container, GAME_STATE.bossX, GAME_STATE.bossY);
        }
    }
}
